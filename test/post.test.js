...
describe('Testing Blog routes', () => {
    ...
    it('should create new blog post.',async()=>{
        ...
        const res=await chai.request(app).post('/posts/').field('title', testingData.title).field('description', testingData.description).field('likes', testingData.likes).field('comments', testingData.comments).set('Authorization', token)
        expect(res.status).to.be.equal(200);
        expect(res.body).to.be.a('object')
    })
    it('should get all blog articles.',async()=>{
        const adminSignin=await chai.request(app).post('/user').send(admin)
        const token = `Bearer ${adminSignin.body.token}`;
        const res1=await chai.request(app).post('/user/login').send(testingData).set('Authorization', token)
